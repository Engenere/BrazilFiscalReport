...
class DanfseParser:
    ...
    def _t(self, node: Element | None, tag: str) -> str:
        """Atalho para get_tag_text com o namespace padrão."""
        return get_tag_text(node, K.URL, tag)

    def _find(self, node: Element | None, path: str) -> Element | None:
        if node is None:
            return None
        return node.find(f"{K.URL}{path}")

    def _city(self, code: str, fallback: str = "") -> str:
        return city_from_ibge(code, self.municipios, fallback)
    ...
    def parse(self) -> dict:
        ...
        emit = self._find(root, "emit") or self._find(inf_nfse, "emit")
        ...
        data["municipal_taxes"] = self._parse_municipal_taxes(
            dps, valores, serv
        )
        ...
        data["complementary_info"] = self._parse_complementary_info(
            serv, dps, data
        )
        return data
    ...
    def _parse_issuer(self, emit, inf_nfse, ender_nac, reg_trib) -> dict:
        ...
        return {
            ...
            "phone": truncate_text(
                format_phone(self._t(emit, "fone")), K.Limits.PHONE
            ),
            "name": truncate_text(
            ...
            "city": self._city(
                self._t(inf_nfse, "cLocEmi"), self._t(inf_nfse, "xLocEmi")
            ),
            "cep": cep,
    ...
    def _parse_party(self, node, with_im: bool = True) -> dict:
        ...
        entity = {
            ...
            or K.EMPTY,
            "phone": truncate_text(
                format_phone(self._t(node, "fone")), K.Limits.PHONE
            ),
    ...
    def _parse_service(self, serv, inf_nfse) -> dict:
        ...
        country = self._iso2_country(
            country_code, self._t(serv, "xPaisPrestacao")
        )
        ...
        return {
            ...
            "description": truncate_text(
                description, K.Limits.SERVICE_DESCRIPTION
            )
            if description
            else K.EMPTY,
            "short_description": truncate_text(
                x_trib, K.Limits.SHORT_DESCRIPTION
            ),
        }

    @staticmethod
    def _iso2_country(code: str, name: str) -> str:
        """Padrão ISO-2 para o país. 1058 (BR) -> 'BR'."""
        if code in ("1058", "76", "BR", ""):
            return "BR"
    ...
    def _parse_municipal_taxes(self, dps, valores, serv) -> dict:
        ...
        if exig is not None:
            ...
            susp_number = truncate_text(
                self._t(exig, "nProcesso"), K.Limits.PROCESS
            )
        else:
        ...
        return {
            ...
            "municipal_benefit_math": money(
                self._t(dps, "vCalcBM") or self._t(dps, "vRedBCBM"),
                self.price_precision,
            )
            if (self._t(dps, "vCalcBM") or self._t(dps, "vRedBCBM"))
            else K.EMPTY,
            "deduct_reduc_amount": money(ded_red, self.price_precision)
            if ded_red
            else K.EMPTY,
            "service_amount": money(v_serv, self.price_precision),
            "discount_unconditioned": money(v_desc_inc, self.price_precision)
            if v_desc_inc
            else K.EMPTY,
            "calculation_basis": money(v_bc, self.price_precision),
            "aliq_applied": f"{format_number(aliq, self.price_precision)}%"
            if aliq
            else K.EMPTY,
            "issqn_retention": retention,
            "issqn_cleared": money(v_issqn, self.price_precision)
            if ret_type == "1"
            else money(0, self.price_precision),
    ...
    def _parse_federal_taxes(self, dps, data) -> dict:
        ...
        result["irrf"] = money(v_irrf, self.price_precision) if v_irrf else K.EMPTY
        result["previdenciary_contribution"] = (
            money(v_cp, self.price_precision) if v_cp else K.EMPTY
        )
        result["social_contribution"] = (
            money(v_csll, self.price_precision) if v_csll else K.EMPTY
        )
        ...
        if self._competence_allows_pis_cofins(data.get("compet_raw", "")):
            ...
            if pis_cofins is not None:
                ...
                result["pis_debit"] = money(pis, self.price_precision)
                result["cofins_debit"] = money(cofins, self.price_precision)
                result["_pis_cofins_total"] = to_float(pis) + to_float(cofins)
        return result

    @staticmethod
    def _competence_allows_pis_cofins(compet_raw: str) -> bool:
        """PIS/COFINS só existem para competência até dez/2026."""
        if not compet_raw:
            return True  # sem competência: não bloqueia (comportamento seguro)
        try:
            year = int(compet_raw.split("T")[0].split("-")[0])
            month = int(compet_raw.split("T")[0].split("-")[1])
    ...
    def _parse_ibs_cbs(self, dps, valores, reg_trib) -> dict:
        ibs_cbs = self._find(dps, "IBSCBS") or self._find(valores, "IBSCBS")
        ...
        result = {
            ...
            "excl_red_bc": money(0, pp),
            "bc_apos_excl": money(0, pp),
            "red_aliq_ibs_cbs": "% / % / %",
            "aliq_ibs_uf_mun": "% / %",
            "aliq_efet_mun_ibs": "%",
            "valor_apurado_mun_ibs": money(0, pp),
            "aliq_efet_est_ibs": "%",
            "valor_apurado_est_ibs": money(0, pp),
            "valor_total_apurado_ibs": money(0, pp),
            "aliq_cbs": "%",
            "aliq_efet_cbs": "%",
            "valor_total_apurado_cbs": money(0, pp),
            "_v_ibs_tot": 0.0,
            "_v_cbs": 0.0,
        }
        if ibs_cbs is None:
        ...
        result["ind_oper_ibge_mun_uf"] = (
            " / ".join(p for p in (ind_op, loc_code, loc_name, uf) if p)
            or K.EMPTY
        )
        ...
        result["excl_red_bc"] = money(excl, pp)
        result["bc_apos_excl"] = money(self._t(ibs_cbs, "vBC"), pp)
        ...
        result["valor_apurado_mun_ibs"] = money(self._t(ibs_cbs, "vIBSMun"), pp)
        result["aliq_efet_est_ibs"] = (
            f"{format_number(self._t(ibs_cbs, 'pAliqEfetUF'), pp)}%"
        )
        result["valor_apurado_est_ibs"] = money(self._t(ibs_cbs, "vIBSUF"), pp)
        ...
        result["valor_total_apurado_ibs"] = money(v_ibs, pp)
        ...
        result["valor_total_apurado_cbs"] = money(v_cbs, pp)
    ...
    def _parse_totals(self, dps, valores, data) -> dict:
        ...
        return {
            "service_amount": money(v_serv, pp),
            "discount_unconditioned": money(v_desc_inc, pp)
            if v_desc_inc
            else K.EMPTY,
            "discount_conditioned": money(v_desc_cond, pp)
            if v_desc_cond
            else K.EMPTY,
            "issqn_retained": money(issqn_retained, pp),
            "total_federal_retentions": money(federal_retentions, pp),
            "total_retentions": money(total_retentions, pp),
            "net_value": money(v_liq, pp),
            "total_ibs_cbs": money(v_ibs_cbs, pp),
            "net_value_ibs_cbs": money(net_plus_ibs_cbs, pp),
            "pis_cofins_debit": money(
                fed.get("_pis_cofins_total", 0.0), pp
            )
    ...
    def _parse_approx_taxes(self, dps) -> dict:
        ...
        return {
            "federal_tax": money(fed, pp) if fed else K.EMPTY,
            "state_tax": money(est, pp) if est else K.EMPTY,
            "municipal_tax": money(mun, pp) if mun else K.EMPTY,
        }
    ...
    def _parse_complementary_info(self, serv, dps, data) -> str:
        ...
        mun = (
            taxes["municipal_tax"]
            if taxes["municipal_tax"] != K.EMPTY
            else "R$ ou %"
        )
        transparency = (
