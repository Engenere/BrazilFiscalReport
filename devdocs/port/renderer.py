...
from .generate_qrcode import build_qr_url, draw_qr_code
...
class Danfse(xFPDF):
    ...
    def _field(self, x, y, w, label, value, label_size=6, value_size=8,
               value_limit=None):
    ...
    def _draw_header(self):
        ...
        self._field(x + 3, sy, cw * 4, "CHAVE DE ACESSO DA NFS-E",
                    d["key_nfse"])
        ...
        self._field(x + cw * 2, sy + 7, cw, "DATA E HORA DA EMISSÃO DA NFS-E",
        ...
        self._field(x + cw * 2, sy + 14, cw, "DATA E HORA DA EMISSÃO DA DPS",
                    f"{d['dt_dps']} {d['hr_dps']}")
        ...
        qr_url = build_qr_url(d["key_nfse"])
        qr_x = x + pw - 17.5
        draw_qr_code(self, qr_url, 0, qr_x, sy)
        self._nt_fonts.apply(self._nt_fonts.small())
        self.set_xy(x=x + cw * 3 - 8, y=sy + 17)
        self.multi_cell(
            w=cw + 6, h=2.4,
            text=("A autenticidade desta NFS-e pode ser verificada pela "
    ...
    def _draw_issuer(self):
        ...
        self._field(x + cw * 2, sy, cw, "Indicador Municipal (Inscrição)",
                    iss["municipal_registration"])
        self._field(x + cw * 3, sy, cw, "Telefone", iss["phone"])

        self._field(x + 3, sy + 7, cw * 2, "Nome / Nome Empresarial",
                    iss["name"], value_limit=cw * 2)
        self._field(x + cw * 2, sy + 7, cw, "Município / Sigla UF", iss["city"])
        self._field(x + cw * 3, sy + 7, cw, "Código IBGE / CEP", iss["ibge_cep"])

        self._field(x + 3, sy + 14, cw * 2, "Endereço", iss["address"],
                    value_limit=cw * 2)
        self._field(x + cw * 2, sy + 14, cw * 2, "E-mail", iss["email"],
                    value_limit=cw * 2)

        self._field(x + 3, sy + 21, cw,
                    "Simples Nacional na Data de Competência", iss["simples"])
        self._field(x + cw * 2, sy + 21, cw * 2,
                    "Regime de Apuração Tributária pelo SN", iss["tax_regim"],
                    value_limit=cw * 2)
    ...
    def _draw_taker(self):
        ...
        self._field(x + 3, sy + 7, cw * 2, "Nome / Nome Empresarial",
                    tk["name"], value_limit=cw * 2)
        self._field(x + cw * 2, sy + 7, cw, "Município / Sigla UF", tk["city"])
        self._field(x + cw * 3, sy + 7, cw, "Código IBGE / CEP", tk["ibge_cep"])

        self._field(x + 3, sy + 14, cw * 2, "Endereço", tk["address"],
                    value_limit=cw * 2)
        self._field(x + cw * 2, sy + 14, cw * 2, "E-mail", tk["email"],
                    value_limit=cw * 2)
    ...
    def _draw_service_provided(self):
        ...
        self._field(x + cw, sy, cw,
                    "Código de Tributação Nacional / Municipal", combined)
        self._field(x + cw * 2, sy, cw, "Código da NBS", sv["nbs_code"])
        local = f"{sv['place_of_provision']} / {sv['country']}"
        self._field(x + cw * 3, sy, cw, "Local da Prestação / Sigla UF / País",
                    local, value_limit=cw)
    ...
    def _draw_taxes(self):
        ...
        if mt.get("no_incidence"):
            ...
            self.cell(w=cw * 3, h=7, text="OPERAÇÃO NÃO SUJEITA AO ISSQN",
                      align="L")
        else:
            self._field(x + cw, sy - 1, cw, "Tipo de Tributação do ISSQN",
                        mt["issqn_tax_short"])
            mun = mt["city"]
            if mt["country"] and mt["country"] not in mun:
                mun = f"{mun} / {mt['country']}"
            self._field(x + cw * 2, sy - 1, cw * 2,
                        "Município / Sigla UF / País de Incidência do ISSQN",
                        mun)

            self._field(x + 3, sy + 11, cw,
                        "Regime Especial de Tributação do ISSQN",
                        mt["special_tax_regim"])
            self._field(x + cw, sy + 11, cw, "Tipo de Imunidade do ISSQN",
                        mt["immunity_type"])
            self._field(x + cw * 2, sy + 11, cw,
                        "Suspensão da Exigibilidade do ISSQN",
                        mt["suspension_issqn"])
            self._field(x + cw * 3, sy + 11, cw, "Número Processo Suspensão",
                        mt["suspension_number"])

            self._field(x + 3, sy + 18, cw, "Benefício Municipal",
                        mt["municipal_benefit"])
            self._field(x + cw, sy + 18, cw, "Cálculo do BM",
                        mt["municipal_benefit_math"])
            self._field(x + cw * 2, sy + 18, cw, "Total Deduções/Reduções",
                        mt["deduct_reduc_amount"])
            self._field(x + cw * 3, sy + 18, cw, "Desconto Incondicionado",
                        mt["discount_unconditioned"])

            self._field(x + 3, sy + 25, cw, "BC ISSQN", mt["calculation_basis"])
            self._field(x + cw, sy + 25, cw, "Alíquota Aplicada",
                        mt["aliq_applied"])
            self._field(x + cw * 2, sy + 25, cw, "Retenção do ISSQN",
                        mt["issqn_retention"])
            self._field(x + cw * 3, sy + 25, cw, "ISSQN Apurado",
                        mt["issqn_cleared"])
        ...
        self._field(x + cw * 2, fy - 1, cw,
                    "Contribuição Previdenciária - Retida",
                    ft["previdenciary_contribution"])
        self._field(x + cw * 3, fy - 1, cw, "Contribuições Sociais - Retidas",
                    ft["social_contribution"])
        self._field(x + 3, fy + 6, cw, "PIS - Débito Apuração Própria",
                    ft["pis_debit"])
        self._field(x + cw, fy + 6, cw, "COFINS - Débito Apuração Própria",
                    ft["cofins_debit"])
        self._field(x + cw * 2, fy + 6, cw,
    ...
    def _draw_ibs_cbs_taxes(self):
        ...
        self._field(x + 3, sy + 6, cw, "Exclusões e Reduções da BC",
                    ic["excl_red_bc"])
        self._field(x + cw, sy + 6, cw, "BC Após Exclusões e Reduções",
                    ic["bc_apos_excl"])
        self._field(x + cw * 2, sy + 6, cw,
                    "Red. Alíquota IBS / Mun / CBS", ic["red_aliq_ibs_cbs"])
        self._field(x + cw * 3, sy + 6, cw, "Alíquota - IBS UF / IBS Mun",
                    ic["aliq_ibs_uf_mun"])

        self._field(x + 3, sy + 13, cw, "Alíq. Efetiva Municipal - IBS",
                    ic["aliq_efet_mun_ibs"])
        self._field(x + cw, sy + 13, cw, "Valor Apurado Municipal - IBS",
                    ic["valor_apurado_mun_ibs"])
        self._field(x + cw * 2, sy + 13, cw, "Alíq. Efetiva Estadual - IBS",
                    ic["aliq_efet_est_ibs"])
        self._field(x + cw * 3, sy + 13, cw, "Valor Apurado Estadual - IBS",
                    ic["valor_apurado_est_ibs"])

        self._field(x + 3, sy + 20, cw, "Valor Total Apurado - IBS",
                    ic["valor_total_apurado_ibs"])
        self._field(x + cw, sy + 20, cw, "Alíquota - CBS", ic["aliq_cbs"])
        self._field(x + cw * 2, sy + 20, cw, "Alíquota Efetiva - CBS",
                    ic["aliq_efet_cbs"])
        self._field(x + cw * 3, sy + 20, cw, "Valor Total Apurado - CBS",
    ...
    def _draw_amount(self):
        ...
        self._field(x + cw, sy - 1, cw, "VALOR DA OPERAÇÃO / SERVIÇO",
                    tv["service_amount"])
        self._field(x + cw * 2, sy - 1, cw, "Desconto Incondicionado",
                    tv["discount_unconditioned"])
        self._field(x + cw * 3, sy - 1, cw, "Desconto Condicionado",
                    tv["discount_conditioned"])

        self._field(x + 3, sy + 6, cw, "Total das Retenções (ISSQN / Federais)",
                    tv["total_retentions"])
        self._field(x + cw, sy + 6, cw, "VALOR LÍQUIDO DA NFS-e",
                    tv["net_value"])
        self._field(x + cw * 2, sy + 6, cw, "Total do IBS/CBS",
                    tv["total_ibs_cbs"])

        # Campo destacado (sombreado) — Valor líquido + IBS/CBS.
        self.shade_rect(x=x + cw * 3, y=sy + 5.5, w=cw, h=11)
        self._field(x + cw * 3, sy + 6, cw,
    ...
    def _draw_complementary_info(self):
        ...
        self.multi_cell(w=self.epw - 6, h=2.5,
                        text=self.data["complementary_info"], align="L")
        self.y = self.get_y() + 1
    ...
    def _draw_canhoto(self):
        ...
        self._block_title(x + 3, sy, "RECEBEMOS O SERVIÇO DESCRITO NESTA NFS-e",
                          shaded=False, col_width=cw * 2)
        self._field(x + 3, sy + 6, cw, "Data de Cientificação", "")
        self._field(x + cw, sy + 6, cw, "Identificação / Assinatura", "")
        self._field(x + cw * 2, sy + 6, cw * 2, "Nº NFS-e / Chave",
                    f"{d['nfse_number']} / {d['key_nfse']}", value_limit=cw * 2)
        self.divider(x + 2, sy + 16, x + pw - 2)
        self.y = sy + 16


def generate_danfse(xml: str | bytes, output_path: str,
                    config: DanfseConfig | None = None) -> str:
