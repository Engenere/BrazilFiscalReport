...
def test_danfe_cancelled_production(tmp_path, load_danfe):
    config = DanfeConfig(
        margins=Margins(top=2, right=2, bottom=2, left=2),
        watermark_cancelled=True,
        product_description_config=ProductDescriptionConfig(
    ...
    danfe = load_danfe("nfe_cancelled_production.xml", config=config)
    pdf_path = get_pdf_output_path("danfe", "danfe_cancelled_production")
    assert_pdf_equal(danfe, pdf_path, tmp_path)


def test_danfe_cancelled_homologation(tmp_path, load_danfe):
    config = DanfeConfig(
        margins=Margins(top=2, right=2, bottom=2, left=2),
        watermark_cancelled=True,
        product_description_config=ProductDescriptionConfig(
    ...
    danfe = load_danfe("nfe_cancelled_homologation.xml", config=config)
    pdf_path = get_pdf_output_path("danfe", "danfe_cancelled_homologation")
    assert_pdf_equal(danfe, pdf_path, tmp_path)
